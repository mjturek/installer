...
func New(logger logrus.FieldLogger, metadata *types.ClusterMetadata) (providers.Destroyer, error) {
	...
	//	logger.Debugf("powervs.New: metadata.ObjectMeta = %v", metadata.ObjectMeta)
	...
	//	logger.Debugf("powervs.New: metadata.ClusterPlatformMetadata.PowerVS.APIKey = %v", metadata.ClusterPlatformMetadata.PowerVS.APIKey)
	...
	return &ClusterUninstaller{
		APIKey:             metadata.ClusterPlatformMetadata.PowerVS.APIKey,
...
func (o *ClusterUninstaller) destroyCluster() error {
	stagedFuncs := [][]struct {
		...
		//		{name: "Jobs", execute: o.destroyJobs},
...
func (o *ClusterUninstaller) loadSDKServices() error {
	...
	o.piSession, err = ibmpisession.New(authenticator,
		region,
		false,
		stageTimeout,
		user.Account,
	...
	o.instanceClient = instance.NewIBMPIInstanceClient(o.piSession, o.ServiceGUID)
	...
	o.imageClient = instance.NewIBMPIImageClient(o.piSession, o.ServiceGUID)
	...
	o.jobClient = instance.NewIBMPIJobClient(o.piSession, o.ServiceGUID)
	if o.jobClient == nil {
		o.Logger.Debugf("loadSDKServices: bxSession = %v", bxSession)
		o.Logger.Debugf("loadSDKServices: tokenRefresher = %v", tokenRefresher)
		o.Logger.Debugf("loadSDKServices: ctrlv2 = %v", ctrlv2)
...
func (o *ClusterUninstaller) timeout(ctx context.Context) bool {
	...
	//	var ret bool
	//	now = time.Now()
	//	difference = deadline.Sub(now).Seconds()
	//	ret = now.After(deadline)
	//	o.Logger.Debugf("timeout: deadline,ok,ret,diff = %v, %v, %v, %v", deadline, ok, ret, difference)
	//	return ret
}
